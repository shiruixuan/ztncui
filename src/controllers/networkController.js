...
exports.index = async function(req, res) {
  ...
  try {
    ...
    res.render('index', {title: 'ztncui 控制器', navigate: navigate, zt_status});
  } catch (err) {
    res.render('index', {title: 'ztncui 控制器',
...
exports.ipAssignmentPools = async function(req, res) {
  ...
  req.checkBody('ipRangeStart', '需要 IP 起始地址').notEmpty();
  req.checkBody('ipRangeStart', 'IP 起始地址需要有效的 IPv4 或 IPv6 地址').isIP();
  req.sanitize('ipRangeStart').escape();
  req.sanitize('ipRangeStart').trim();
  req.checkBody('ipRangeEnd', '需要 IP 结束地址').notEmpty();
  req.checkBody('ipRangeEnd', 'IP结束地址需要有效的 IPv4 或 IPv6 地址').isIP();
  ...
  if (errors) {
    try {
      ...
      res.render('ipAssignmentPools', {title: 'IP 分配池', navigate: navigate, error: '解析网络的网络详细信息时出错 ' + req.params.nwid + ': ' + err});
    }
  } else {
    try {
      ...
      res.render('ipAssignmentPools', {title: 'IP 分配池', navigate: navigate, error: '应用 IP 分配池时出错' + req.params.nwid + '：' + err});
    }
  }
}
...
exports.routes = async function (req, res) {
  ...
  req.checkBody('target', '目标网络必须是有效的 CIDR 格式')
    .custom(value => {
...
exports.ipAssignmentPool_delete = async function (req, res) {
  ...
  try {
    ...
    navigate.whence = '/controller/network/' + network.nwid;
    res.render('IP 分配池', {title: 'IP 分配池', navigate: navigate, ipAssignmentPool: ipAssignmentPool, network: network});
  } catch (err) {
    res.render('IP 分配池', {title: 'IP 分配池', navigate: navigate, error: '删除网络的 IP 分配池时出错' + req.params.nwid + ': ' + err});
  }
}
...
exports.v4AssignMode = async function (req, res) {
  ...
  try {
    ...
    navigate.whence = '/controller/network/' + network.nwid;
    res.render('v4AssignMode', {title: 'V4 分配模式', navigate: navigate, network: network});
  } catch (err) {
    res.render('v4AssignMode', {title: 'V4 分配模式', navigate: navigate, error: '为网络应用 V4 分配模式时出错' + req.params.nwid + ': ' + err});
  }
}
...
exports.v6AssignMode = async function (req, res) {
  ...
  try {
    ...
    navigate.whence = '/controller/network/' + network.nwid;
    res.render('v6AssignMode', {title: 'V6 分配模式', navigate: navigate, network: network});
  } catch (err) {
    res.render('v6AssignMode', {title: 'V6 分配模式', navigate: navigate, error: '为网络应用 V6 分配模式时出错' + req.params.nwid + '：' + err});
  }
}
...
exports.dns = async function (req, res) {
  ...
  try {
    ...
    res.render('dns', {title: 'DNS', navigate: navigate, error: '更新网络的 DNS 时出错' + req.params.nwid + '：' + err});
  }
}
...
exports.members = async function(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    req.checkBody('id', '成员 ID 是必需的').notEmpty();
...
exports.assign_ip = async function(req, res) {
  ...
  req.checkBody('ipAddress', '需要 IP 地址').notEmpty();
  req.checkBody('ipAddress', 'IP 地址必须是有效的 IPv4 或 IPv6 地址').isIP();
  req.checkBody('ipAddress', 'IP 地址必须位于托管路由内')
    .custom(value => {
